# Mirrord Outgoing
Another feature that is enabled by default is outgoing traffic & remote DNS resolving, for example let's say you want to check what is the response form the database because you encountered some unwanted behavior or just want to develop something new and want to see the real results
...
So what's happening is your local service accesses the the database from the internal network of the Kubernetes so you should be able to navigate to http://localhost:30080 though you receive the result from the server instance that is running on the cluster you should still see the **following log in the local machine**
